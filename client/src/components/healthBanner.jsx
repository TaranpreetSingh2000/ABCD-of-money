...
import { fetchHealthCardData } from "@/services/healthCardbanner.service";

const HealthCard = async () => {
  const healthcardData = await fetchHealthCardData();

  return (
    <div className="flex flex-col gap-5 md:w-[70%]">
      {healthcardData.data.map((item, index) => (
        <div
          key={index}
          className="relative gap-5 rounded-3xl overflow-hidden flex flex-col md:flex-row items-start justify-between md:items-center px-[30px] py-[28px]"
        >
          <img
            src={`${process.env.NEXT_STRAPI_URL}${item.banner.image.url}`}
